...
if [ "$#" -lt 3 ]; then
  ...
  echo "(normally invoked via the tui's prefix+v -> clip-to-pane.sh)" >&2
  exit 2
fi
repo_root="$1"
dest_dir="$2"
...
if [ -n "$dest_dir" ]; then
  ...
  # The v2 engine mounts the workspace at a fixed target
  # (internal/model/names.go WorkspaceTarget).
  container_path="/workspace/$dest_dir/$file_name"
else
...
if [ -z "$dest_dir" ]; then
  if [ -z "$container" ]; then
    echo "No running container for $repo_root (vibe up first, or use: vibe clip DIR)" >&2
    exit 1
  fi
